...
static const int LAST_POW_BLOCK = 15060;
...
public:
    ...
    {
        // cypher: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // cypher: get transaction coin age
...
public:
    ...
    // cypher: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // cypher: two types of block: proof-of-work or proof-of-stake
    ...
    // cypher: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // cypher: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // cypher: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // cypher: block index flags
    enum  
    {
